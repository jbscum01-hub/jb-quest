...
} = require('../../services/discordConfig.service');
const {
  buildProfessionPanelEmbed
} = require('../../builders/embeds/professionPanel.embed');
const {
  buildProfessionPanelComponents
} = require('../../builders/components/professionPanel.components');
    ...
    try {
      ...
      if (!channelId) {
        results.push({
          professionCode,
          ok: false,
          reason: 'ไม่พบ channel id ใน config'
        });
        continue;
      }
      ...
      if (!channel) {
        results.push({
          professionCode,
          ok: false,
          reason: 'ไม่พบ channel ใน Discord'
        });
        continue;
      }
