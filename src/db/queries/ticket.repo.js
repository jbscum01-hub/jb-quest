...
async function updateTicketStatus(ticketId, status, closeInfo = {}, client) {
  ...
  const result = await db.query(
    ...
    SET ticket_status = $2,
        closed_at = CASE WHEN $2 IN ('COMPLETED', 'CANCELLED', 'CLOSED') THEN NOW() ELSE closed_at END,
        closed_by = COALESCE($3, closed_by),
        close_remark = COALESCE($4, close_remark),
        updated_at = NOW()
    WHERE ticket_id = $1
    RETURNING *
    `,
    [ticketId, status, closeInfo.closedBy || null, closeInfo.closeRemark || null]
  );
